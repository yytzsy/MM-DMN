...
from model.TripleAttentiveModal_DMN import TripleAttentiveModal_DMN
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(config=sess_config) as sess:
            ...
            while dataset.has_train_batch:
                ...
                _, loss, acc, reg_loss, log_loss, attentions_modal = sess.run(
                    [model.train, model.loss, model.acc, model.reg_loss, model.log_loss, model.attentions_modal], feed_dict)
                total_loss += loss
                total_acc += acc
                if batch_idx % 100 == 0:
...
def test(epoch, dataset, config, log_dir, question_type_dict):
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(config=sess_config) as sess:
            ...
            result.to_json(os.path.join(log_dir, 'prediction_'+str(epoch)+'.json'), 'records')
...
def main():
    ...
    if args.mode == 'train':
        ...
        for epoch in range(0, 30):
            ...
            test_acc = test(epoch, dataset, config, args.log, question_type_dict)
            # val_acc = val(epoch, dataset, config, args.log)

    elif args.mode == 'test':
        print('start test.')
        test(-1,dataset, config, args.log, question_type_dict)
