...
class MSRVTTQA(object):
    ...
    def get_train_batch(self):
        ...
        start = self.train_batch_idx[self.current_bucket] * self.train_batch_size
        ...
        for i in range(self.train_batch_size):
            ...
            question_len_batch.append(len(qid))
            ...
            vgg_conv5_batch.append(self.video_feature.root.vgg_conv5[video_ids[i]])
            vgg_conv5_3_batch.append(self.video_feature.root.vgg_conv5_3[video_ids[i]])
            audio_batch.append(self.video_feature.root.mfcc[video_ids[i]])
        ...
        return vgg_batch, c3d_batch, vgg_conv5_batch, vgg_conv5_3_batch, audio_batch, question_batch, answer_batch, question_len_batch
    ...
    def get_val_example(self):
        ...
        vgg_conv5 = self.video_feature.root.vgg_conv5[video_id]
        vgg_conv5_3 = self.video_feature.root.vgg_conv5_3[video_id]
        audio = self.video_feature.root.mfcc[video_id]

        self.val_example_idx += 1
        if self.val_example_idx == self.val_example_total:
            self.has_val_example = False

        return vgg, c3d, vgg_conv5, vgg_conv5_3, audio, question, answer, question_len
    ...
    def get_test_example(self):
        ...
        return vgg, c3d, vgg_conv5, vgg_conv5_3, audio, question, answer, example_id, question_len
...
class MSVDQA(object):
    ...
    def __init__(self, train_batch_size, preprocess_dir):
        ...
        self.video_feature = tables.open_file(os.path.join(preprocess_dir, 'video_feature_20.h5'))

        # contains encode, for fast generate batch by saving encode time
        self.train_qa = pd.read_json(os.path.join(preprocess_dir, 'train_qa_encode.json'))
        self.val_qa = pd.read_json(os.path.join(preprocess_dir, 'val_qa_encode.json'))
        self.test_qa = pd.read_json(os.path.join(preprocess_dir, 'test_qa_encode.json'))

        # init train batch setting
        self.train_qa['question_length'] = self.train_qa.apply(lambda row: len(row['question'].split()), axis=1)
    ...
    def get_train_batch(self):
        ...
        for i in range(self.train_batch_size):
            ...
            vgg_conv5_batch.append(self.video_feature.root.vgg_conv5[video_ids[i] - 1])
            vgg_conv5_3_batch.append(self.video_feature.root.vgg_conv5_3[video_ids[i] - 1])
        ...
        return vgg_batch, c3d_batch, vgg_conv5_batch, vgg_conv5_3_batch, question_batch, answer_batch, question_len_batch
    ...
    def get_val_example(self):
        ...
        vgg_conv5 = self.video_feature.root.vgg_conv5[video_id - 1]
        vgg_conv5_3 = self.video_feature.root.vgg_conv5_3[video_id-1]

        self.val_example_idx += 1
        if self.val_example_idx == self.val_example_total:
            self.has_val_example = False

        return vgg, c3d, vgg_conv5, vgg_conv5_3, question, answer, question_len
    ...
    def get_test_example(self):
        ...
        return vgg, c3d, vgg_conv5, vgg_conv5_3, question, answer, example_id, question_len