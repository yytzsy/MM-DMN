...
class VideoVGGExtractor(object):
    ...
    def extract(self, path):
        ...
        feature = self.sess.run(
            self.vgg16.conv5_3, feed_dict={self.inputs: frames})
        return feature
...
class VideoC3DExtractor(object):
    ...
    def __init__(self, clip_num, sess):
        ...
        #saver.restore(sess, os.path.join(path, 'sports1m_finetuning_ucf101.model'))
        saver.restore(sess, '/home/yuanyitian/video_qa/Gradually_Refined_Attention/util/sports1m_finetuning_ucf101.model')
        #self.mean = np.load(os.path.join(path, 'crop_mean.npy'))
        self.mean = np.load('/home/yuanyitian/video_qa/Gradually_Refined_Attention/util/crop_mean.npy')
        self.sess = sess
