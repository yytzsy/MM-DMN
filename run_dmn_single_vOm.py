...
from model.DMN_single_VM import DMN_single_VM
...
def train(epoch, dataset, config, log_dir):
    ...
    with tf.Graph().as_default():
        model = DMN_single_VM(model_config)
        ...
        with tf.Session(config=sess_config) as sess:
            ...
            while dataset.has_train_batch:
                vgg, c3d, vgg_conv5, vgg_conv5_3, question, answer, question_len = dataset.get_train_batch()
                tmp_batch_size = len(question_len)
                input_len = np.zeros(tmp_batch_size)+20
                feed_dict = {
                    model.video_feature: vgg,
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(config=sess_config) as sess:
            ...
            while dataset.has_val_example:
                vgg, c3d, vgg_conv5, vgg_conv5_3, question, answer, question_len = dataset.get_val_example()
                input_len = 20
                feed_dict = {
                    model.video_feature: [vgg],
                    model.question_encode: [question],
