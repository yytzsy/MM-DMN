        ...
        with tf.variable_scope("multimodal_memory", initializer=tf.contrib.layers.xavier_initializer()):
            ...
            for i in range(self.num_hops):
                ...
                with tf.variable_scope("hop_%d" % i):
                    prev_memory = tf.layers.dense(tf.concat([prev_memory, vgg_episode*c3d_episode*mfcc_episode, q_vec], 1), self.hidden_size, activation=tf.nn.relu)
            output = prev_memory
