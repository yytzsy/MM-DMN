...
class Multimodal_DMN_VM(object):
    ...
    def __init__(self, config):
        ...
        self.c3d_video_feature = None
        self.vgg_video_feature = None
    ...
    def get_attention(self, q_vec, prev_memory, prev_other_episode, fact_vec, reuse):
        """Use question vector and previous memory to create scalar attention for current fact"""
        with tf.variable_scope("attention", reuse=reuse):
            features = [fact_vec*q_vec,fact_vec*prev_memory,fact_vec*prev_other_episode,
                        tf.abs(fact_vec - q_vec),tf.abs(fact_vec - prev_memory),tf.abs(fact_vec - prev_other_episode)]
    ...
    def generate_episode(self, save_attentions, memory, other_episode, q_vec, fact_vecs, hop_index, namescope):
        """Generate episode by applying attention to current fact vectors through a modified GRU"""
        with tf.variable_scope(namescope):
            attentions = [tf.squeeze(self.get_attention(q_vec, memory, other_episode, fv, bool(hop_index) or bool(i)), axis=1)
    ...
    def build_inference(self):
        ...
        with tf.variable_scope("multimodal_memory", initializer=tf.contrib.layers.xavier_initializer()):
            ...
            print(np.shape(pre_vgg_episode))
            print(np.shape(pre_c3d_episode))
            for i in range(self.num_hops):
                # get a new episode
                print('==> generating multimodal episode', i)
                c3d_episode = self.generate_episode(self.attentions_c3d, prev_memory, pre_vgg_episode, q_vec, c3d_vecs, i,'c3d_episode')
                vgg_episode = self.generate_episode(self.attentions_vgg, prev_memory, pre_c3d_episode, q_vec, vgg_vecs, i,'vgg_episode')
                ...
                with tf.variable_scope("hop_%d" % i):
                    prev_memory = tf.layers.dense(tf.concat([prev_memory, c3d_episode, vgg_episode, q_vec], 1),self.hidden_size,activation=tf.nn.relu)
            output = prev_memory
