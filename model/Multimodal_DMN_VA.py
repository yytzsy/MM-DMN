...
class Multimodal_DMN_VA(object):
    ...
    def build_inference(self):
        ...
        with tf.variable_scope("multimodal_memory", initializer=tf.contrib.layers.xavier_initializer()):
            ...
            for i in range(self.num_hops):
                ...
                mfcc_episode = self.generate_episode(self.attentions_mfcc, prev_memory, pre_vgg_episode, q_vec, mfcc_vecs, i,'mfcc_episode')
                vgg_episode = self.generate_episode(self.attentions_vgg, prev_memory, pre_mfcc_episode, q_vec, vgg_vecs, i,'vgg_episode')
                pre_vgg_episode = vgg_episode
                pre_mfcc_episode = mfcc_episode
                # untied weights for memory update
                with tf.variable_scope("hop_%d" % i):
                    prev_memory = tf.layers.dense(tf.concat([prev_memory, mfcc_episode*vgg_episode, q_vec], 1),self.hidden_size,activation=tf.nn.relu)
            output = prev_memory
