...
def extract_vgg(video_directory):
    ...
    with tf.Graph().as_default(), tf.Session(config=sess_config) as sess:
        ...
        for i in range(1, 1971):
            name = id_dict['vid' + str(i)]
            video_path = os.path.join(video_directory, name + '.avi')
            print('[VGG]', video_path)
            tmp = extractor.extract(video_path)
            print np.shape(tmp)
            vgg_features.append(tmp)
...
def extract_vgg_video_feature(video_directory, feature_path):
    h5file = tables.open_file(
...
def main():
    ...
    # os.makedirs('/mnt/data/yuanyitian/videoQA/dmn/data/msvd_qa')

    extract_vgg_video_feature(os.path.join(sys.argv[1], ''),
                         '/mnt/data/yuanyitian/videoQA/dmn/data/msvd_qa/video_feature_20_conv5.h5')

    # extract_video_feature('/mnt/data/yuanyitian/MSVD/Youtube2Text/YouTubeClips',
